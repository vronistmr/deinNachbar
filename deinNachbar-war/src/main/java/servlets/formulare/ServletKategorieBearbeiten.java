	...
	private void update(int kategorieID, String NeueKategorie) throws ServletException {
		...
		try (Connection con = ds.getConnection();
				PreparedStatement pstmt = con.prepareStatement(
						"UPDATE anzeige SET anzeige.kategorie = ? WHERE anzeige.kategorie = (SELECT kategorie.kategorie FROM kategorie WHERE kategorie.kategorieID = ?)")) {
		...
		try (Connection con = ds.getConnection();
				PreparedStatement pstmt = con
						.prepareStatement("UPDATE kategorie SET kategorie = ? WHERE kategorieID = ?")) {
